...
class StorageService {
  ...
  static Map<String, String> getAIExportPaths() {
    try {
      final box = Hive.box(_settingsBox);
      return {
        'openai': box.get('ai_openai_path', defaultValue: '') as String,
        'anthropic': box.get('ai_anthropic_path', defaultValue: '') as String,
        'google': box.get('ai_google_path', defaultValue: '') as String,
      };
    } catch (e) {
  ...
  /// Save AI export folder path
  static Future<void> setAIExportPath(String provider, String path) async {
    try {
      final box = Hive.box(_settingsBox);
      await box.put('ai_${provider}_path', path);
    } catch (e) {
